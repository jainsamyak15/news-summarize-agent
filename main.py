...
class NewsSummarizerAgent:
    ...
    def fetch_news(self, url_or_params: Union[str, Dict], max_articles: int = 5) -> List[Dict]:
        ...
        if isinstance(url_or_params, str):
            ...
            params = url_or_params.copy()  # Make a copy to avoid modifying the original
    ...
    async def summarize_article(self, article: Dict) -> NewsArticle:
        ...
        # Generate summary using LLMChain - run in a thread pool to not block
...
# Dependency to get the news summarizer agent
@asynccontextmanager
...
async def summarize_news(request: SummarizeRequest):
    async with get_news_summarizer() as agent:
        # Determine if we're using URL or params
...
async def get_top_headlines(request: SummarizeRequest):
    async with get_news_summarizer() as agent:
        # If params are provided, ensure the endpoint is set to top-headlines
        if request.params:
            params = request.params.copy()
            params["endpoint"] = "top-headlines"
            query = params
        # If URL is provided, check if it's already for top-headlines
        elif request.url:
            ...
            # Default to US top headlines if nothing specific is provided
